...
function DeliveryInfo({
  ...
  const [
    backgroundNoticaticionBack1,
    setBackgroundNoticaticionBack1,
  ] = useState('');
  const [timerBack1, setTimerBack1] = useState(1);
  const [
    backgroundNoticaticionBack2,
    setBackgroundNoticaticionBack2,
  ] = useState('');
  const [timerBack2, setTimerBack2] = useState(1);

  const [
    backgroundNoticaticionFrontOn1,
    setBackgroundNoticaticionFrontOn1,
  ] = useState('');
  const [timerFrontOn1, setTimerFrontOn1] = useState(1);
  const [
    backgroundNoticaticionFrontOn2,
    setBackgroundNoticaticionFrontOn2,
  ] = useState('');
  const [timerFrontOn2, setTimerFrontOn2] = useState(1);
  ...
  useEffect(() => {
    ...
    const onOpenNotification = async (notify) => {
      ...
      console.log('Parando Recusa passiva');
      console.log('Parou Recusa passiva');
      //playSoundNotification();
      //checkNewDelivery();
    };
  ...
  function backgroundMessagemRecusapassiva() {
    if (Object.keys(newDelivery).length > 1) {
      ...
      function timer2() {
        contador -= timerBack2;
        setTimerBack2(contador);
      }
      console.log('Estou nos teste recusa passiva background');
      // call timer ever 1000 milliseconds
      setBackgroundNoticaticionBack2(
        BackgroundTimer.runBackgroundTimer(timer2, 1000),
      );
    } else {
      /* TIMER 1*/
      // this function run in backgroud
  ...
  function backgroundMessagemRecusapassivaFront() {
    ...
    if (Object.keys(newDelivery).length > 1) {
      ...
      // this function run in backgroud
      function timer1() {
        contador -= timerFrontOn1;
        setTimerFrontOn1(contador);
      }
      let contador = 60;
      console.log('Estou nos teste recusa passiva foreground 1');
      // call timer ever 1000 milliseconds
      setBackgroundNoticaticionFrontOn1(
        BackgroundTimer.runBackgroundTimer(timer1, 1000),
      );
      ...
      // this function run in backgroud
      function timer2() {
        contador -= timerFrontOn2;
        setTimerFrontOn2(contador);
      }
      console.log('Estou nos teste recusa passiva foreground 2');
      // call timer ever 1000 milliseconds
      setBackgroundNoticaticionFrontOn2(
        BackgroundTimer.runBackgroundTimer(timer2, 1000),
      );
  ...
  useEffect(() => {
    if (timerBack1 === 0) {
      RecusaPassiva();
      BackgroundTimer.stopBackgroundTimer(backgroundNoticaticionBack1);
      return;
    }
    if (timerFrontOn1 === 0) {
      RecusaPassiva();
      BackgroundTimer.stopBackgroundTimer(backgroundNoticaticionFrontOn1);
      return;
    }
    if (timerBack2 === 0) {
      RecusaPassiva();
      BackgroundTimer.stopBackgroundTimer(backgroundNoticaticionBack2);
      return;
    }
    if (timerFrontOn2 === 0) {
      RecusaPassiva();
      BackgroundTimer.stopBackgroundTimer(backgroundNoticaticionFrontOn2);
      return;
    }
  }, [timerBack1, timerFrontOn1, timerBack2, timerFrontOn2]);
  ...
  //Recusar Delivery
  async function recuseDelivery(cd_entrega, index) {
    //Teste Guilherme
              ...
              {newDelivery.map((item, index) => {
                return (
                  <View
                    ...
                    <View style={styles.rowMiniCard}>
                      <TouchableOpacity
                        onPress={() => recuseDelivery(item.cd_entrega, index)}
                      ...
                      <TouchableOpacity
                        ...
                        <Text style={styles.buttonText}>
                          {' '}
                          Aceitar{' '}
                          {timerBack1 === 1 && index === 0
                            ? ''
                            : timerBack1 + 's'}
                          {timerFrontOn1 === 1 && index === 0
                            ? ''
                            : timerFrontOn1 + 's'}
                          {timerFrontOn2 === 2 && index === 1
                            ? ''
                            : timerFrontOn2 + 's'}
                        </Text>
                      </TouchableOpacity>
